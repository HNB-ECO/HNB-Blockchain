...
func Evaluate(pri *ecdsa.PrivateKey, h hash.Hash, m []byte) (index [32]byte, proof []byte) {
	...
	fmt.Printf("w1 %v\n", curve.Marshal(pri.PublicKey.X, pri.PublicKey.Y))
	...
	//tt, _ := elliptic.Unmarshal(curve, vrf)
	//if tt == nil {
	//	panic("1111")
	//}
	...
	var b bytes.Buffer
	fmt.Printf("w3 %v\n", curve.Marshal(curve.Gx, curve.Gy))
	...
	fmt.Printf("w6 %v\n", vrf)
	b.Write(vrf)
	fmt.Printf("w7 %v\n", curve.Marshal(rGx, rGy))
	b.Write(curve.Marshal(rGx, rGy))
	fmt.Printf("w8 %v\n", curve.Marshal(rHx, rHy))
...
func ProofToHash(pk *ecdsa.PublicKey, h hash.Hash, m, proof []byte) (index [32]byte, err error) {
	...
	fmt.Printf("r6 %v\n", vrf)
	b.Write(vrf)
	fmt.Printf("r7 %v\n", curve.Marshal(tksGx, tksGy))
	b.Write(curve.Marshal(tksGx, tksGy))
	fmt.Printf("r8 %v\n", curve.Marshal(tksHx, tksHy))
