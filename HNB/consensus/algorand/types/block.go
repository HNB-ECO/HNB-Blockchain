...
type Block struct {
	...
	*ValidatorInfo `json:"validator_info"`
}

// MakeBlock returns a new block with an empty header, except what can be computed from itself.
// It populates the same set of fields validated by ValidateBasic
func MakeBlock(blkNum uint64, txs []Tx, commit *Commit) *Block {
	block := &Block{
		Header: &Header{
			BlockNum: blkNum,
			Time:     time.Now().UTC(),
...
func (h *Header) Hash() []byte {
	...
	return merkle.SimpleHashFromMap(map[string]merkle.Hasher{
		...
		"VRFValue":    aminoHasher(h.BlkVRFValue),
		"VRFProof":    aminoHasher(h.BlkVRFProof),
	})
...
type Data struct {
	...
	// Txs that will be applied by state @ block.BlockNum+1.
...
func (commit *Commit) PreCommitVP() string {
	...
	for i, preCommit := range commit.Precommits {
		if preCommit != nil {
			strSlice[i] = strconv.Itoa(preCommit.ValidatorIndex)
		}
	}
...
// BlockNum returns the height of the commit
...
func (commit *Commit) Hash() cmn.HexBytes {
	...
	if commit.hash == nil {
		...
		for i, precommit := range commit.Precommits {
			bs[i] = cmn.AminoHasher(precommit)
		}
...
func (blockID BlockID) Key() string {
	//	bz, err := Codec.MarshalBinaryBare(blockID.PartsHeader)
	//FIXME
	bz, err := cmn.Hash256Byte2(blockID.PartsHeader)
	if err != nil {
		panic(err)
	}
...
func (h hasher) Hash() []byte {
	bz, err := Codec.MarshalBinaryBare(h.item)
	if err != nil {
		return nil
	}

	digest, err := msp.Hash256(bz)
	if err != nil {
		return nil
	}
